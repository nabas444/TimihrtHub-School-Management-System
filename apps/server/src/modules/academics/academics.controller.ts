...
export const listAssignments = async (
  ...
  try {
    ...
    const { subjectId, classId, termId } = req.query as Record<
      string,
      string | undefined
    >;
    const { assignments, total } = await AcademicsService.listAssignments(
      req.user.schoolId,
      { subjectId, classId, termId, page, limit },
    );
    sendSuccess(
      res,
      assignments,
...
export const createAssignment = async (
  ...
  try {
    ...
    sendCreated(
      res,
      await AcademicsService.createAssignment(req.user.schoolId, req.user.id, {
        ...data,
...
export const listExams = async (
  ...
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = Math.min(parseInt(req.query.limit as string) || 20, 100);
    const { classId, subjectId, termId } = req.query as Record<
      string,
      string | undefined
    >;
    const { exams, total } = await AcademicsService.listExams(
      req.user.schoolId,
      { classId, subjectId, termId, page, limit },
    );
