...
import { Bell, Menu, Sun, Moon, Search, X, Check, Languages } from 'lucide-react';
...
export default function Topbar() {
  ...
  const { data: notifData } = useQuery({
    ...
    queryFn: () => api.get('/notifications?unread=true&limit=10').then((r) => r.data.data),
  ...
  const NOTIF_ICON = { GRADE: '📊', ASSIGNMENT: '📚', ATTENDANCE: '📋', BEHAVIOUR: '⚠️', FEE: '💰', EXAM: '📝', CHAT: '💬', MEETING: '📅', GENERAL: '🔔', ANNOUNCEMENT: '📢' };
  ...
  return (
    <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex items-center px-4 gap-3 flex-shrink-0">
        ...
        {notifOpen && (
          <div className="absolute right-0 top-12 w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 z-50 animate-slide-in overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={clsx('px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer border-b border-gray-50 dark:border-gray-800/50', !n.isRead && 'bg-primary-50/50 dark:bg-primary-900/10')}
                    onClick={() => { setNotifOpen(false); navigate('/dashboard'); }}
                  >
                    <div className="flex items-start gap-3">
                      <span className="text-lg mt-0.5">{NOTIF_ICON[n.type] ?? '🔔'}</span>
            ...
            <div className="px-4 py-2 border-t border-gray-100 dark:border-gray-800">
              <button onClick={() => { setNotifOpen(false); navigate('/dashboard'); }} className="text-xs text-primary-600 hover:underline w-full text-center">
      ...
      {/* User avatar */}
      <div className="w-9 h-9 rounded-full bg-primary-600 flex items-center justify-center text-white font-bold text-sm cursor-pointer" onClick={() => navigate('/settings/profile')}>
